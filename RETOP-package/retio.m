...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Attention: au dela de la version 7.10 incluse retio est neutralisé par defaut. Pour l'utiliser, faire [prv,vmax]=retio([],5000i)
...
% dans le même repertoire,les noms de fichiers sont précédés d'un prefixe
...
% [prv,vmax]=retio([]); retourne le vmax actuel                                     |
%------------------------------------------------------------------------------------
...
% a_conserver=retio; puis retio(a_conserver,-4);
%
...
persistent vmax z v6;vm=vmax;if isempty(v6);[prv,vers,v6]=retversion;if vers>=7.01;vmax=inf;end;end;
if ~isfinite(vmax) & nargin>1 & k==1;return;end;
