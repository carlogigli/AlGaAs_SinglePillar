...
%  retfig(2,15*i):    charge  20000015.fig la met en noir ,l'imprime et l'efface
%  x=retfig(2,-15*i): charge  20000015.fig dans figure(x) 
...
eval(['xx=open(''',nomfig(fg,ii),'.fig'');']);set(xx,'position',get(0,'ScreenSize'));%caxis([-18,0]);
if nargin>4;
switch(numel(cx));
...
ccx=cx*ccx./(1+(cx-1)*ccx);colormap(interp1(1:mm,cm,1+(mm-1)*ccx,'pchip'));
otherwise
colormap(cx);
end;
...
if ~isempty(nom);set(xx,'DoubleBuffer','on');pause(.1);frame=getframe(gcf);cine=addframe(cine,frame);end;
