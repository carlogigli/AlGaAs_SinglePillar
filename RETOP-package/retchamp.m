...
%    points calcules:     linspace(0,sum(abs(tab(:,1))),abs(tab(1,3)))
%
%   ELEMENTS FINIS
...
% cas particulier: y=retchamp(tab,nan) donne les valeurs de y avec les points de gauss
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
[e,yy,ww,o,s]=sources_multiples(init,a,sh,sb,inc,x,y,h,nargout);
return;end;	
...
if size(init,2)>=3;% ajout d'une couche dans tab     [tab,k]=retchamp(tab,h); ------------------------
    if ~isfinite(a);% calcul du y avec les points de gauss
    tab=init;
    if size(tab,2)==3;tab(:,4)=1;end;
    e=[];bas=0;
    for ii=size(tab,1):-1:1;
    if imag(tab(ii,3))==0;e=[e,bas+retgauss(0,tab(ii,1),tab(ii,3),tab(ii,4))];end;
    bas=bas+tab(ii,1);
    end;
    return;end;    
...
if init{end}.granet==1;inc=retgranet(init,inc);end;% transformation de granet   
e=retchamp0(init,retio(a),sh,sb,inc);return;%pour: e=retchamp(init,a,inc,sens,x) 
...
if init{end}.granet==1;x=retgranet(init,x);end;% transformation de granet   
...
if a{end}.type==8;% Cercles Aureliens
ix=interp1(xx,1:length(xx),mod(x,pas(1)),'nearest');ix(isnan(ix))=length(xx);
iy=interp1(yy,1:length(yy),mod(y,pas(2)),'nearest');iy(isnan(iy))=length(yy);
for ii=1:3;ee(1,:,:,ii)=uu{2}(ix,iy);ee(1,:,:,ii+3)=uu{1}(ix,iy);end;
Zeta=reshape(uu{7}(ix,iy),1,length(ix),length(iy));Ksi=reshape(uu{8}(ix,iy),1,length(ix),length(iy));% sert pour le calcul propre des champs
...
if ~iscell(fey);% version non symetrisée
En=reshape(ret2ff(kx,ky,fey*v(1:2*n),betaa),1,length(kx),length(ky))./ee(1,:,:,4);
Et=reshape(ret2ff(kx,ky,fex*v(1:2*n),betaa),1,length(kx),length(ky));
e(nn,:,:,1)=Ksi.*En-conj(Zeta).*Et;	
e(nn,:,:,2)=Zeta.*En+conj(Ksi).*Et;	
else;  % version symetrisée 
En1=reshape(ret2ff(kx,ky,fey{1}*v(1:2*n),betaa),1,length(kx),length(ky))./ee(1,:,:,4);
Et1=reshape(ret2ff(kx,ky,fex{1}*v(1:2*n),betaa),1,length(kx),length(ky));
En2=reshape(ret2ff(kx,ky,fey{2}*v(1:2*n),betaa),1,length(kx),length(ky))./ee(1,:,:,4);
Et2=reshape(ret2ff(kx,ky,fex{2}*v(1:2*n),betaa),1,length(kx),length(ky));
e(nn,:,:,1)=.5*(Ksi.*En1-conj(Zeta).*Et1+conj(Ksi).*En2-Zeta.*Et2);	
e(nn,:,:,2)=.5*(Zeta.*En1+conj(Ksi).*Et1+conj(Zeta).*En2+Ksi.*Et2);	
end;

if ~isempty(fhy);
if ~iscell(fhy);% version non symetrisée
Hn=reshape(ret2ff(kx,ky,fhy*v(2*n+1:4*n),betaa),1,length(kx),length(ky))./ee(1,:,:,1);
Ht=reshape(ret2ff(kx,ky,fhx*v(2*n+1:4*n),betaa),1,length(kx),length(ky));
e(nn,:,:,4)=Ksi.*Hn-conj(Zeta).*Ht;	
e(nn,:,:,5)=Zeta.*Hn+conj(Ksi).*Ht;	
else;  % version symetrisée 
Hn1=reshape(ret2ff(kx,ky,fhy{1}*v(2*n+1:4*n),betaa),1,length(kx),length(ky))./ee(1,:,:,1);
Ht1=reshape(ret2ff(kx,ky,fhx{1}*v(2*n+1:4*n),betaa),1,length(kx),length(ky));
Hn2=reshape(ret2ff(kx,ky,fhy{2}*v(2*n+1:4*n),betaa),1,length(kx),length(ky))./ee(1,:,:,1);
Ht2=reshape(ret2ff(kx,ky,fhx{2}*v(2*n+1:4*n),betaa),1,length(kx),length(ky));
e(nn,:,:,4)=.5*(Ksi.*Hn1-conj(Zeta).*Ht1+conj(Ksi).*Hn2-Zeta.*Ht2);	
e(nn,:,:,5)=.5*(Zeta.*Hn1+conj(Ksi).*Ht1+conj(Zeta).*Hn2+Ksi.*Ht2);	
end;

else;% mu constant
e(nn,:,:,4)=reshape(ret2ff(kx,ky,v(2*n+1:3*n),betaa),1,length(kx),length(ky));	
e(nn,:,:,5)=reshape(ret2ff(kx,ky,v(3*n+1:4*n),betaa),1,length(kx),length(ky));	
end;	
...
if iscell(npts);npts{1}=sum(real(tab(:,1)))-npts{1}(end:-1:1);end;% npts{1}= points en entrée
[e,y,w,o]=retchamp_0D(init,flipud(tab),retrenverse(Tb,3),retrenverse(Th,2),[0,inc(1)],npts);
e0=e(end:-1:1,:,:,:,:);e0(:,:,2,:,:)=-e0(:,:,2,:,:);y=sum(real(tab(:,1)))-y(end:-1:1);w=w(end:-1:1);o=o(end:-1:1);
else;e0=[];
end;
...
function [tab,sh,sb,Lh,Lb]=sources_multiples(tab,xs,sh,sb,ah,ab,source,ns,Nargout);
htab=sum(tab(:,1));
...
if Nargout==1;sh=[];sb=[];Lh=[];Lb=[];return;end;% pour gain de temps (ex retop boite)
if ~isempty(fs_b);
Lb=-xs(fs_b(1));
